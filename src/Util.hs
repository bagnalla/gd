...
-- Can these be defined for general bifunctors instead?
bimapM :: Applicative m => (a -> m c) -> (b -> m d) ->
          (a, b) -> m (c, d)
bimapM f g (x, y) = pure (,) <*> f x <*> g y

bimapM' :: Applicative m => (a -> m b) -> (a, a) -> m (b, b)
bimapM' f = bimapM f f

firstM :: Monad m => (a -> m c) -> (a, b) -> m (c, b)
firstM = flip bimapM return

secondM :: Monad m => (b -> m c) -> (a, b) -> m (a, c)
secondM = bimapM return
